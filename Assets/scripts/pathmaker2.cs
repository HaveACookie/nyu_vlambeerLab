...
//			Destroy my game object; 		// self destruct if I've made enough tiles already
		...
		{	
			...
			if (GameManager.Instance.tileCount >= tileMax)
			{
				Destroy(gameObject);
			}
	...
	void Raycasting()
	{
		Debug.DrawLine(castStart.position, castEnd.position, Color.magenta);
		isHit = Physics.Linecast(castStart.position, castEnd.position, 1 << LayerMask.NameToLayer(("Floor")));
	}
